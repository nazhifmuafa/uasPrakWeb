...
if (isset($_POST['insert'])) {
	...
	if ($gambar !="") {
        ...
        if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
            move_uploaded_file($file_tmp, 'gambar_kue/'.$nama_gambar_baru);
            $query = "INSERT INTO produk (id_kategori, nama, keterangan, gambar, harga, status) VALUES 
            ('$id_kategori', '$nama_kue', '$deskripsi', '$nama_gambar_baru', '$harga', '$status')";
	...
	if($gambar != "") {
	  ...
	  if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
					move_uploaded_file($file_tmp, 'gambar_kue/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
					  ...
					  $query  = "UPDATE produk SET id_kategori = '$id_kategori', nama='$name', keterangan = '$deskripsi', harga = '$harga', gambar = '$nama_gambar_baru', status = '$status'";
		...
		$query  = "UPDATE produk SET id_kategori = '$id_kategori', nama='$name', keterangan = '$deskripsi', harga = '$harga', status = '$status'";
					  $query .= "WHERE id_kue = '$id'";
					  $result = mysqli_query($conn, $query);
					  // periska query apakah ada error
					  if(!$result){
